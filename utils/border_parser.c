...
/*   Updated: 2025/08/25 00:58:24 by aobshatk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_up(t_map *map, int i)
{
	...
	{
		if ((map->up->cols[i] == ' ' || map->up->cols[i] == '	') && map->cols[i] == '0')
			map->cols[i] = 'a';
	}
...
static void	check_down(t_map *map, int i)
{
	...
	{
		if ((map->down->cols[i] == ' ' || map->down->cols[i] == '	') && map->cols[i] == '0')
			map->cols[i] = 'a';
	}
...
static void	check_right(t_map *map, int i)
{
	if (i < map->num_cols - 2)
	{
		if ((map->cols[i + 1] == ' ' || map->cols[i + 1] == '	') && map->cols[i] == '0')
		{
			map->cols[i] = 'a';
		}
		else
	...
	if (i == map->num_cols - 3 && map->cols[i + 1] == '0')
		map->cols[i] = 'a';
	if (i == map->num_cols - 3)
	{
